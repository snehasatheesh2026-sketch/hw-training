...
url = "https://www.matalanme.com/jo_en/men/footwear/sandals-flip-flops"
...
try:
    ...
    if match:
         ...
         print("CategoryTree with UID not found")
...
# Change category ID
variables["filter"]["category_uid"]["in"] = uid
...
for item in items:
    ...
    url = product.get("url_key", "")
    product_id = product.get("id", "")
    quantity = product.get("qty_left_in_stock",'')
    ...
    for section in attrs:
        ...
        if section.get("title") == "Specifications":
            ...
            for spec in section.get("children", []):
                ...
                product_details[spec["label"]] = spec["value"]
