...
class Parser:
    ...
    def __init__(self):
        ...
        self.target_collection = self.db[MONGO_COLLECTION_PRODUCT_DATA]
        
        self.queue = ''    # queue connect
        self.mongo = client  # mongo connect

    def start(self):
        """start code"""

        # Fetching product info dictionaries from MongoDB source collection
        metas = list(self.source_collection.find({}))
        print(f"DEBUG: Found {len(metas)} product items to process from MongoDB.")
        ...
        for meta in metas:
            ...
            print(f"DEBUG: Requesting Product URL -> {url}")
            try:
    ...
    def parse_item(self, url, response, meta):
       ...
       # CLEAN
        ...
        # self.mongo.process(product_item, collection=MONGO_COLLECTION_PRODUCT_DATA)  # Mongo insert using items

        logging.info(item)
        try:
            # self.target_collection.insert_one(item)
            product_item = ProductdataItem(**item)
            # # self.mongo.process(product_item, collection=MONGO_COLLECTION_PRODUCT_DATA)
            product_item.save()
            print("saved")
