                    ...
                    for product in products:
                        ...
                        if varient_id and unique_id:
                            ...
                            try:
                                ...
                                descri_response = self.session.get(
                                                                  url,
                                                                 params=self.details_params,
                                                                 headers=self.header_description,
                                                                timeout=30,
                                                                )
                                print(f"Description [{unique_id}] -> {descri_response.status_code}")
                                ...
                                if descri_response.status_code == 200:
                                    ...
                                    if html_description:
                                       ...
                                       print(f"No description for {unique_id}")
                                # time.sleep(0.2)
                                in_url = self.ingredients_url.format(unique_id, varient_id)

                                ingredient_response = self.session.get(
                                                                    in_url,
                                                                     params=self.details_params,
                                                                      headers=self.ingredient_header,
                                                                       timeout=30,
                                                                       )
                                print(f"Ingredients [{unique_id}] -> {ingredient_response.status_code}")
                                ...
                                feature_respo = self.session.get(
                                                     feature_url,
                                                     params=self.details_params,
                                                    headers=self.header_parameter,
                                                    timeout=30,
                                                    )
                                print(f"Features [{unique_id}] -> {feature_respo.status_code}")
                                
                                if feature_respo.status_code == 200:
                                ...
                                allergens_response = self.session.get(
                                         allergen_url, 
                                         params= self.details_params, 
                                         headers= self.allergens_headers
                                                           )
                                print(f"allergens [{unique_id}] -> {allergens_response.status_code}")
                                allergensDetailed = ""

                                if allergens_response.status_code == 200:
                                    all_data = allergens_response.json()
                                    raw_allergensDetailed = all_data.get('allergensDetailed',[])
                                    if raw_allergensDetailed:
                                ...
                                print(f"Error for product {unique_id}: {e}")
