            ...
            while True:
                ...
                try:
                    ...
                    for product in products:
                        ...
                        pack_size = f"{str(product['amount'])}{product['volumeLabelShort']}"
                        ...
                        raw_descripitions = product.get('descriptionShort','')

                        de = raw_descripitions.replace("<br>", " ").replace("<li>", " ,")

                        descripitions = re.sub(r'<[^>]+>', '', de)
                        ...
                        if product_id:
                            ...
                            try:
                                ...
                                if response.status_code ==200:
                                    ...
                                    info =  datas.get('additionalInformation', [{}])[0]
                                    ...
                                    manufacturersAddress = info.get('productInformation', {}).get('manufacturersAddress','')
                              ...
                              pass
                        descripitions = descripitions.replace("<li>", " ").replace("</li>", " ").replace("<ul>"," ").replace("</ul>"," ")

                        descripitions = "".join(descripitions.split()).replace('\n', ' ').replace('\r', ' ').replace('|', ' ').strip()


                        descripitions.replace("\n"," ,").removesuffix('</li></ul>').removesuffix(' ')

                        manufacturersAddress = manufacturersAddress.replace("\n"," ,")
                        ...
                        yield {
                            ...
                            "regular_price":round(regular_raw_price/100,2) or "",
                            "price_per_unit":round(price_per_unit/100,2) or "",
                            ...
                            "describition":f"{product.get('descriptionLong','')}{descripitions}" or "",
...
if __name__ == "__main__":
    ...
    with open(csv_filename, "w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(
            file, 
            fieldnames=["unique_id","cateogry_name" ,"brand","product_name","Country_of_production","regular_price","diet_tags","pack_size","package_type","price_per_unit","images","nutrition","manufacturersAddress","Phone_no","address","email","variety","breadcrumbs","pdp_url","describition","Unit_of_Measure", "competitor_name"]
        )
        writer.writeheader()
